...
const CACHE      = 'manyuka-v5';          // bump on every deploy
...
self.addEventListener('activate', e => {
  e.waitUntil(
    Promise.all([
      ...
      // ← OPT: claim clients inside the same waitUntil so activation is
      //         fully complete before any fetch events are handled
      self.clients.claim(),
    ])
  );
...
self.addEventListener('fetch', e => {
  ...
  // Supabase API — network only, empty-array fallback
  ...
  e.waitUntil(
    self.registration.showNotification(data.title, options).then(() => {
      if (data.notifId && data.userId) {
        // ← OPT: matchAll result is already a promise; no need to nest .then inside .then
        return self.clients
