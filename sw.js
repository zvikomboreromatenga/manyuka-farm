...
function _zimTimeStrSW(){
  return new Date().toLocaleTimeString('en-US', {timeZone:'Africa/Harare', hour:'numeric', minute:'2-digit', hour12:true});
}
// Zimbabwe (CAT) is a fixed UTC+2 with no DST — a simple offset shift
// reliably gives Zimbabwe wall-clock values regardless of the device's
// own timezone.
function _zimNowPartsSW(){
...
async function checkScheduledDigest(creds, headers) {
  const zim = _zimNowPartsSW();
  const hh = zim.hour, mm = zim.minute, dow = zim.dow;
  const today = zim.dateStr;

  const pastDigestTime = hh > 20 || (hh === 20 && mm >= 30);
  const pastSummaryTime = dow === 0 && (hh > 14 || (hh === 14 && mm >= 30));
  const pastNudgeTime = hh >= 7;
  if (!pastDigestTime && !pastSummaryTime && !pastNudgeTime) return; // nothing to do yet, skip the extra fetches

  // Who is this device's user, and what's their role?
  const ur = await fetchWithTimeout(
    `${creds.sb}/rest/v1/app_users_public?id=eq.${encodeURIComponent(creds.userId)}&select=id,display_name,username,role,todo_enabled`,
    { headers }
  );
  if (!ur.ok) return;
  ...
  // ── Daily digest — admin + main_admin only, after 8:30pm, once per day ──
  if (pastDigestTime && (me.role === 'admin' || me.role === 'main_admin')) {
    ...
    if (!alreadyLocally && !(await digestAlreadySentToday(creds, headers, 'system-daily-digest', today))) {
      try {
        ...
        const [salesR, layerSalesR, expR, eggsR, layerEggsR, birdMortR, goatMortR, incubatorR, vaccR, tickR, ordersR, pendingR] = await Promise.all([
          fetchWithTimeout(`${creds.sb}/rest/v1/sales?select=item,buyer,amount,amount_paid,fully_paid,payment_due_date,date&order=created_at.desc&limit=500`, { headers }),
          fetchWithTimeout(`${creds.sb}/rest/v1/layer_sales?select=amount,date&order=created_at.desc&limit=500`, { headers }),
          fetchWithTimeout(`${creds.sb}/rest/v1/expenses?select=amount,date&order=created_at.desc&limit=500`, { headers }),
          fetchWithTimeout(`${creds.sb}/rest/v1/eggs?date=eq.${today}&select=id,count&limit=1`, { headers }),
          fetchWithTimeout(`${creds.sb}/rest/v1/layer_eggs?date=eq.${today}&select=id,count&limit=1`, { headers }),
          fetchWithTimeout(`${creds.sb}/rest/v1/bird_mortality?date=eq.${today}&select=count,cause`, { headers }),
          fetchWithTimeout(`${creds.sb}/rest/v1/goat_mortality?date=eq.${today}&select=count,cause`, { headers }),
        ...
        const layerSales = layerSalesR.ok ? await layerSalesR.json() : [];
        const expenses = expR.ok ? await expR.json() : [];
        const eggsRow = eggsR.ok ? await eggsR.json() : [];
        const layerEggsRow = layerEggsR.ok ? await layerEggsR.json() : [];
        const baLogged = eggsRow.length > 0;
        const layerLogged = layerEggsRow.length > 0;
        const birdMort = birdMortR.ok ? await birdMortR.json() : [];
        const goatMort = goatMortR.ok ? await goatMortR.json() : [];
        ...
        const salesToday = [...sales, ...layerSales].filter(r => r.date === today);
        const expToday = expenses.filter(r => r.date === today);
        const mortToday = [...birdMort, ...goatMort].filter(r => r.cause !== 'cock_sale' && r.cause !== 'sold');
        ...
        const lines = [`It's ${_zimTimeStrSW()} in Zimbabwe, here is your daily summary, ${myName}:\n`];
        lines.push(`💰 Sales: ${salesToday.length?salesToday.length+' recorded, '+fmtMoneySW(salesToday.reduce((s,r)=>s+(r.amount||0),0))+' total':'none today'}`);
        lines.push(`💸 Expenses: ${expToday.length?expToday.length+' recorded, '+fmtMoneySW(expToday.reduce((s,r)=>s+(r.amount||0),0))+' total':'none today'}`);
        lines.push(`🥚 BA Eggs: ${baLogged?fmtEggsSW(eggsRow[0].count||0):'not logged yet'}`);
        lines.push(`🥚 Layer Eggs: ${layerLogged?fmtEggsSW(layerEggsRow[0].count||0):'not logged yet'}`);
        if (mortToday.length) lines.push(`🪦 Losses today: ${mortToday.reduce((s,r)=>s+(r.count||0),0)}`);
  ...
  // ── Weekly summary — everyone, Sunday after 2:30pm, once per week ──
  if (pastSummaryTime) {
    ...
    if (!alreadyLocally && !(await digestAlreadySentToday(creds, headers, 'system-weekly-summary', today))) {
      try {
        ...
        const lines = [`It's ${_zimTimeStrSW()} in Zimbabwe, here is your weekly summary — what happened on the farm this past week:\n`];
  ...
  // ── Morning to-do nudge — todo_enabled users, after 8am, once per day ──
  if (pastNudgeTime && me.todo_enabled) {
    try {
      const taskR = await fetchWithTimeout(
        `${creds.sb}/rest/v1/admin_tasks?owner_id=eq.${encodeURIComponent(creds.userId)}&date=eq.${today}&select=id&limit=1`,
        { headers }
      );
      const hasTaskToday = taskR.ok ? (await taskR.json()).length > 0 : true; // assume yes if check failed, avoid false nudges
      if (!hasTaskToday && !(await digestAlreadySentToday(creds, headers, 'system-todo-nudge', today))) {
        await sendDigestMessage(creds, headers, 'system-todo-nudge', creds.userId, myName,
          '📋 Morning check-in',
          `It's ${_zimTimeStrSW()} in Zimbabwe, here is your morning check-in, ${myName}! You haven't created a to-do list for today yet — head to your Home screen to add what you're working on today.`);
        await self.registration.showNotification('📋 Morning check-in — Manyuka Farm', {
          body: 'Add today\'s to-do list when you get a chance.',
          icon: '/icon-192.png', badge: '/icon-192.png',
          tag: 'todo-nudge-' + today, data: { link: 'dash' }, vibrate: [200,100,200],
        });
      }
    } catch (err) { console.warn('[SW] todo nudge failed:', err); }
  }
}
...
async function runBackgroundCheck() {
  ...
  try {
    ...
    // ── Scheduled digests (daily 8:30pm admin summary, Sunday 2:30pm all-hands) ──
    ...
    const r = await fetchWithTimeout(   // ← OPT: timeout guard
      `${creds.sb}/rest/v1/messages?recipient_id=eq.${creds.userId}&is_read=eq.false&select=id,sender_id,sender_name,subject,body,created_at`,
      { headers }
    );
    if (!r.ok) return;
    ...
    // Digest messages (system-daily-digest / system-weekly-summary) already
    // got their own tailored notification from checkScheduledDigest() above
    // — skip them here so the same message doesn't notify twice.
    const isDigestSender = id => id === 'system-daily-digest' || id === 'system-weekly-summary';

    // Show a notification per new message — preview real content, not just
    // the subject line, which is often blank/generic for quick chats.
    for (const msg of newMsgs) {
      if (!isDigestSender(msg.sender_id)) {
        const preview = (msg.body || msg.subject || '').replace(/\s+/g, ' ').trim().slice(0, 100);
        await self.registration.showNotification('💬 ' + (msg.sender_name || 'Someone') + ' — Manyuka Farm', {
      ...
      notifiedSet.add(msg.id); // mark seen either way, so digest messages don't get re-checked next cycle
    }
...
self.addEventListener('push', e => {
  ...
  e.waitUntil(
    self.registration.showNotification(data.title, options).then(async () => {
      // Record message-type pushes into the SAME tracking store the
      // periodic background check reads from (manyuka-notified-ids) —
      // otherwise that check has no way to know this message was already
      // pushed in real time, and will keep re-notifying about it on every
      // cycle for as long as it stays unread (which could be a while).
      if (data.type === 'message' && data.notifId && data.notifId.startsWith('msg-')) {
        const bareId = data.notifId.slice(4);
        try {
          const storedIds = (await dbGet('manyuka-notified-ids')) || [];
          if (!storedIds.includes(bareId)) {
            await dbSet('manyuka-notified-ids', [...storedIds, bareId].slice(-MAX_NOTIF_IDS));
          }
        } catch (_) {}
      }
...
Deno.serve(async (req) => {
  ...
  // Digest messages get their own tailored notification from the service
  // worker's periodic check — skip here to avoid notifying twice for the
  // same message (once via this real-time webhook, once via that check).
  if (record.sender_id === 'system-daily-digest' || record.sender_id === 'system-weekly-summary') {
    return new Response('skipped (digest — handled elsewhere)');
  }
  ...
  const preview = (record.body || record.subject || '').replace(/\s+/g, ' ').trim().slice(0, 100);
  const pushPayload = JSON.stringify({
    title: '💬 ' + (record.sender_name || 'Someone') + ' — Manyuka Farm',
    body:  preview + (preview.length >= 100 ? '…' : ''),
    link:  'messages',
    type:  'message',
  });
